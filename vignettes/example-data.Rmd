...
# Data example creations

Authorize from secrets
...
`get_github_metrics()` returns this kind of data. 

```{r}
get_example_data("gh_repo_metrics")
...
`get_repos_metrics()` returns this kind of data. 
...
```{r}
get_example_data("all_ga_metric_list")
class(all_ga_metric_list)
str(all_ga_metric_list)
```
...
```{r}
get_example_data("multiple_gforms")
